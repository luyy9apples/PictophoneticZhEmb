...
//AddingComp
char pinyin_file[MAX_STRING],char2pinyin_file[MAX_STRING];
char output_pinyin[MAX_STRING];
...
//AddingComp
long long pinyin_max_size = PINYIN_SIZE, pinyin_size = 0;
...
void DestroyVocab(){
  ...
  //AddingBySR
  for(a = 0; a < pinyin_size; a++){
    if(pinyin_array[a].pinyin_val != NULL)
      free(pinyin_array[a].pinyin_val);
  }
...
void ReadPinyin() {
  FILE *fin;
  fin = fopen(pinyin_file, "rb");
  if (fin == NULL){
    fprintf(stderr,"ERROR : pinyin file not found!\n");
    exit(1);
  }
  while(1){
    char word[MAX_STRING];
    ReadWord(word, fin);
    if (feof(fin)) break;
    if(pinyin_size + 2 >= pinyin_max_size){
      pinyin_max_size += PINYIN_SIZE;
      pinyin_array = (struct pinyin *)realloc(pinyin_array,pinyin_max_size * sizeof(struct pinyin));
    }
    unsigned int word_len = strlen(word) + 1;
    pinyin_array[pinyin_size].pinyin_val = (char *)calloc(word_len, sizeof(char));
    strcpy(pinyin_array[pinyin_size].pinyin_val, word);
    pinyin_size++; 
  }
  fclose(fin);
}
...
//AddingComp
int GetPinyinIndex(char *pinyin){
  for(int i = 0; i < pinyin_size; i++){
    if(strcmp(pinyin, pinyin_array[i].pinyin_val) == 0)
      return i;
  }
  return -1;
}
...
void LearnCharComponentsFromFile() {  
  ...
  while ((read = getline(&line, &len, fin)) != -1){
    ...
    if(wstr[0]>MAX_CHINESE || wstr[0]<MIN_CHINESE) {
      //printf("\nwstr[0] out of range: %S %X",wstr,wstr[0]);
      continue;
    }
    ...
    while(pch != NULL){
      ...
      if(pch_index != -1)
        ...
        wchar_t wstrt[MAX_STRING];
        unsigned int wlent = mbstowcs(wstrt, pch, MAX_STRING);
        //printf("\nget pch_index -1: %s %X",pch,wstrt[0]);
      }
      pch = strtok_r(NULL," \n",&save_ptr);
    }
  }
  ...
  //AddingComp
  fin = fopen(char2pinyin_file,"rb");
  if(fin == NULL){
    fprintf(stderr,"ERROR: char2pinyin file not found!\n");
    exit(1);
  }
  ...
  while ((read = getline(&line, &len, fin)) != -1){
    ...
    int id = wstr[0] - MIN_CHINESE;
    //printf("%d\n",id );
    //char2comp[id].pinyin = calloc(1, sizeof(int));           //

    int tmp_cnt = 0;
    pch = strtok_r(NULL," \n",&save_ptr);
    while(pch != NULL){
      int pch_index = GetPinyinIndex(pch);
      if(pch_index != -1)
        //printf("%d\n", pch_index);
        char2comp[id].pinyin = pch_index;
      pch = strtok_r(NULL," \n",&save_ptr);
    }
  }
  if(line)
...
void InitNet(){
  ...
  //AddingBySR
  a = posix_memalign((void **)&synpinyin, 128, (long long)pinyin_size * layer1_size * sizeof(real));
  if (synpinyin == NULL) {printf("Memory allocation failed\n"); exit(1);} 
  ...
  //AddingComp
  for (b = 0; b < layer1_size; b++) for (a = 0; a < pinyin_size; a++)
    synpinyin[a * layer1_size + b] = (rand() / (real)RAND_MAX - 0.5) / layer1_size;
}
...
void *TrainModelThread(void *id) {
  ...
  //AddingComp
  long long *pinyin_id_list = calloc(MAX_SENTENCE_LENGTH, sizeof(long long));
  int char_list_cnt = 0, comp_list_cnt = 0;
  //AddingComp
  int pinyin_list_cnt = 0;
  ...
  //AddingComp
  real *neupinyin = (real *)calloc(layer1_size,sizeof(real));

  real *neucomp_grad = (real *)calloc(layer1_size,sizeof(real));
  //AddingComp
  real *neupinyin_grad = (real *)calloc(layer1_size,sizeof(real));
  ...
  while (1) {   
    ...
    //AddingComp
    for (c = 0; c < layer1_size; c++) neupinyin[c] = 0;

    for (c = 0; c < layer1_size; c++) neucomp_grad[c] = 0;
    //AddingComp
    for (c = 0; c < layer1_size; c++) neupinyin_grad[c] = 0;
    ...
    for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
      ...
      for (c = 0; c < vocab[last_word].character_size; c++) {
        ...
        if (pos_type == 1 || pos_type == 3) {
          ...
          //AddingComp
          pinyin_id = char2comp[char_id].pinyin;
          pinyin_id_list[pinyin_list_cnt++] = pinyin_id;
          for (e = 0; e < layer1_size; e++) neupinyin[e] += synpinyin[e + pinyin_id * layer1_size];
        }
      }
      cw++;
    }
    ...
    if (cw) {
      if (average_sum == 1) {       // the context is represented by the average of the surrounding vectors
        for (c = 0; c < layer1_size; c++) {
          ...
          //AddingComp
          if (pinyin_list_cnt > 0)
            neupinyin[c] /= pinyin_list_cnt;
        }
      }
      // negative sampling
      for (d = 0; d < negative + 1; d++) {
        ...
        if (join_type == 1) {    // sum loss composition model
          ...
          if (f4 > MAX_EXP)
            ...
            g4 = (label - expTable[(int)((f4 + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
          ...
          for (c = 0; c < layer1_size; c++) {
            ...
            //AddingComp
            neupinyin_grad[c] += g4 * syn1neg[c + l2];

          }
          //update syn1neg AddingComp
          for (c = 0; c < layer1_size; c++)
            syn1neg[c + l2] += g1 * neu1[c] + g2 * neuchar[c] + g3 * neucomp[c] + g4 * neupinyin[c] ;
        }
        else if (join_type == 2) { // average context composition model  
          real f = 0, g = 0;
          //AddingComp
          for (c = 0; c < layer1_size; c++)
            f += (neu1[c] + neuchar[c] + neucomp[c] + neupinyin[c] ) * syn1neg[c + l2];
          if (f > MAX_EXP)
            g = (label - 1) * alpha;
          else if (f < -MAX_EXP)
            g = (label - 0) * alpha;
          ...
          for (c = 0; c < layer1_size; c++) {
            ...
            //AddingComp
            neupinyin_grad[c] += g * syn1neg[c + l2];
            neuchar_grad[c] += g * syn1neg[c + l2];
          }
          //AddingBySR
          for (c = 0; c < layer1_size; c++)
            syn1neg[c + l2] += g * (neu1[c] + neuchar[c] + neucomp[c] + neupinyin[c]);
        }
      }
      ...
      //AddingComp
      for (a = 0; a < pinyin_list_cnt; a++) {
        pinyin_id = pinyin_id_list[a];
        for (c = 0; c < layer1_size; c++){
          if (average_sum == 1)
            synpinyin[c + pinyin_id * layer1_size] += neupinyin_grad[c] / pinyin_list_cnt;
          else
            synpinyin[c + pinyin_id * layer1_size] += neupinyin_grad[c];
        }
      }
    }
    sentence_position++;
...
void TrainModel(){
  ...
  //AddingComp
  if (strlen(output_pinyin)){
    fo = fopen(output_pinyin, "wb");
    if (fo == NULL){
      fprintf(stderr, "Cannot open %s: permission denied\n", output_pinyin);
    }
    fprintf(fo, "%lld %lld\n", pinyin_size, layer1_size);
    for(a = 0; a < pinyin_size; a++){
      fprintf(fo, "%s ", pinyin_array[a]);
      if (binary)
        for (b = 0; b < layer1_size; b++) fwrite(&synpinyin[a * layer1_size + b], sizeof(real), 1, fo);
      else
        for (b = 0; b < layer1_size; b++) fprintf(fo, "%lf ", synpinyin[a * layer1_size + b]);
      fprintf(fo, "\n");
    }
    fclose(fo);
  }
...
int main(int argc, char **argv) {
  ...
  //AddingBySR
  if ((i = ArgPos((char *)"-output-pinyin", argc, argv)) > 0) strcpy(output_pinyin, argv[i + 1]);
  ...
  //AddingBySR
  if ((i = ArgPos((char *)"-pinyin", argc, argv)) > 0) strcpy(pinyin_file, argv[i + 1]);
  if ((i = ArgPos((char *)"-char2pinyin", argc, argv)) > 0) strcpy(char2pinyin_file, argv[i + 1]);
  ...
  //AddingBySR
  pinyin_array = (struct pinyin *)calloc(pinyin_max_size, sizeof(struct pinyin));
