...
class FastText {
  ...
  public:
	...
	void substotune(Model&, real, const std::vector<std::vector<int32_t> >&, const std::vector<int32_t>&);
...
void FastText::train(const Args args) {
	...
	if ((args_->model == model_name::skipgram) || (args_->model == model_name::cbow) || (args_->model == model_name::subword)) {
		...
		ifs.close();
	} else if (args_->model == model_name::substoke || args_->model == model_name::substotune) {
		if (args_->infeature == "") {
			throw std::invalid_argument("substoke and substotune must be have infeature file [-infeature]");
		}
		...
		std::cout << "args_model_success" << std::endl;
		dict_->readFromFile(ifs, infeature);
		std::cout << "args_dictionary_success" << std::endl;
		ifs.close();
		infeature.close();
	}
...
void FastText::substotune(Model& model, real lr, const std::vector<std::vector<int32_t> >& source, const std::vector<int32_t>& target) {
	std::uniform_int_distribution<> uniform(1, args_->ws);
	for (int32_t w = 0; w < target.size(); w++) {
		int32_t boundary = uniform(model.rng);
		const std::vector<int32_t>& ngrams = source[w];
...
void FastText::trainThread(int32_t threadId) {
	...
	while (tokenCount_ < args_->epoch * ntokens) {
		...
		}else if (args_->model == model_name::substotune) {
			localTokenCount += dict_->getLine(ifs, sourceType, source, target, model.rng);
			substotune(model, lr, source, target);
		}
...
void FastText::saveVectors() {
	...
	if ((nwords > 0 && nfeatures > 0 && args_->model == model_name::substoke) 
		...
		|| (nwords > 0 && nfeatures > 0 && args_->model == model_name::substotune)) {
		std::string outfile;
	...
	if ((ntargets > 0 && args_->model == model_name::substoke)||(ntargets > 0 && args_->model == model_name::substotune)) {
		std::ofstream ofs(args_->output + ".vec");
		if (!ofs.is_open()) {
