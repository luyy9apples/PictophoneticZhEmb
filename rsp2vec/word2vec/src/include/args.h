...
enum class model_name : int { skipgram = 1, cbow, subword, substoke, substotune};
...
void Args::parseArgs(const std::vector<std::string>& args) {
	...
	}else if ( command == "substotune") {
		model = model_name::substotune;
	}
	...
	if ((model == model_name::substoke||model == model_name::substotune)&& infeature == "") {
